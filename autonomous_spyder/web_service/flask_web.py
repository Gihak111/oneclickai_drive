...
<div style="margin-top:12px;font-family:sans-serif">
  ...
  <div style="margin-top:8px;display:flex;gap:10px;justify-content:center">
    <button onmousedown="press('f')" onmouseup="release('f')" ontouchstart="press('f')" ontouchend="release('f')">발사 (F)</button>
  </div>
</div>
...
document.addEventListener("keydown", (e) => {
  ...
  if (k.toLowerCase() === 'f') { pressed.add(k.toLowerCase()); postKeys(); }
});
document.addEventListener("keyup", (e) => {
  ...
  if (k.toLowerCase() === 'f') { pressed.delete(k.toLowerCase()); postKeys(); }
});
