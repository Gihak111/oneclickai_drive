...
조립이 끝나고 스크립트를 종료(Ctrl+C)하면 토크를 해제한다. 이후 flask_web.py를
실행하면 캘리브레이션 값(calibration.json)으로 다시 자세를 잡고 토크를 켠다.
...
print("(조립이 끝나면 Ctrl+C로 종료하세요. 종료 시 토크가 해제됩니다.)")

try:
    while True:
        time.sleep(1)
except KeyboardInterrupt:
    print("\n종료합니다. 토크를 해제합니다 (손으로 자세를 만질 수 있습니다).")
    ser.write(b"T\n")
    ser.flush()
    time.sleep(0.1)
    ser.close()
