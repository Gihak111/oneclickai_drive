"""
PCA9685 서보 모터 제어 모듈
서보 4개 (채널 0: Base, 1: Shoulder, 2: Elbow, 3: Gripper)를 개별 제어
"""
import time
...
try:
    ...
    print(f"[servo_cont] PCA9685 초기화 실패: {e}")
    print("[servo_cont] 서보 제어 비활성화 상태로 실행합니다.")
    kit = None
...
def init_servos():
    ...
    print(f"[servo_cont] 서보 초기화 완료: {servo_angles}")
...
def move_servo(channel, direction):
    ...
    """
    if not PCA9685_AVAILABLE:
        print(f"[servo_cont] PCA9685 미연결 - servo[{channel}] 요청 무시")
        return
    ...
    print(f"[servo_cont] servo[{channel}] ({JOINTS[channel]}) = {new_angle}°")
...
if __name__ == '__main__':
    init_servos()
    print("서보 테스트: 각 서보를 70° → 90° → 110° → 90° 로 이동")
